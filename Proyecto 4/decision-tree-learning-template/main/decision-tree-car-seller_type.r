...
for (i in 1:10){
  ...
  modelo <- rpart(formula = seller_type ~km_driven + owner + selling_price, data = training_data)
  prediccion <- predict(modelo, test_data, type = "class")
  resPrediccion <- table(test_data$seller_type, prediccion)
